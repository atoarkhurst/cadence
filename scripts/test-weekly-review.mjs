...
try {
  ...
  await act(a)
  assert.equal((await db.query("update profiles set display_name='Ato' where id=$1 returning display_name",[a])).rows[0].display_name,'Ato')
  await act(b)
  assert.equal((await db.query("select display_name from profiles where id=$1",[a])).rows[0].display_name,'Ato')
  assert.equal((await db.query("update profiles set display_name='Changed' where id=$1 returning id",[a])).rows.length,0)
  console.log('PASS: names persist, are visible to partners, and cannot be changed by another account.')
  await act(b)
